...
class SpreadsheetInputController extends Controller
{
                    ...
                    foreach ($studentData['dates'] ?? [] as $date => $cellData) {
                        ...
                        $studentHafalanRecords = collect($allHafalanRecords->get($studentId, []))
                            ->filter(function ($rec) use ($targetDates) {
                                $dateVal = $rec->submitted_at instanceof Carbon ? $rec->submitted_at->format('Y-m-d') : substr((string) $rec->submitted_at, 0, 10);
                        ...
                        $studentUmmiRecords = collect($allUmmiRecords->get($studentId, []))
                            ->filter(function ($rec) use ($targetDates) {
                                $dateVal = $rec->tanggal instanceof Carbon ? $rec->tanggal->format('Y-m-d') : substr((string) $rec->tanggal, 0, 10);
    ...
    private function saveHafalanRecords(int $studentId, int $teacherId, string $date, array $cellData, Collection $existingRecords, Collection $surahs): void
    {
    ...
    private function saveUmmiRecords(int $studentId, int $teacherId, string $date, array $cellData, Collection $existingRecords, Collection $surahs): void
    {
        $existingRecordIds = $existingRecords->pluck('id')->toArray();
        $processedRecordIds = [];
