                    ...
                    <form action="{{ route('tahfizh-exams.store') }}" method="POST" class="space-y-4" @submit="validateForm($event)">
                        ...
                        <div>
                            ...
                            <select id="teacher_id" name="teacher_id" required class="block w-full rounded-lg border-zinc-300 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white text-sm focus:border-indigo-500 focus:ring-indigo-500 transition">
                                @php
                                    $currentUserTeacherId = auth()->user()->teacherProfile?->id;
                                @endphp
                                @foreach ($teachers as $teacher)
                                    <option value="{{ $teacher->id }}" @selected(old('teacher_id', $currentUserTeacherId) == $teacher->id)>
                                        {{ $teacher->user?->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date" id="exam_date" name="exam_date" value="{{ old('exam_date', date('Y-m-d')) }}" required class="block w-full rounded-lg border-zinc-300 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white text-sm focus:border-indigo-500 focus:ring-indigo-500 transition" />
                            @error('exam_date')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div x-show="examType === 'juz'" x-cloak class="bg-indigo-50/50 dark:bg-indigo-950/20 p-3 rounded-lg border border-indigo-100 dark:border-indigo-900/40">
                            ...
                            <select id="juz" name="juz" :required="examType === 'juz'" class="block w-full rounded-lg border-zinc-300 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white text-sm focus:border-indigo-500 focus:ring-indigo-500 transition">
                                ...
                                @for ($j = 1; $j <= 30; $j++)
                                    <option value="{{ $j }}" @selected(old('juz') == $j)>
                                        Juz {{ $j }}
                                    </option>
                                @endfor
                            </select>
                        ...
                        <div x-show="examType === 'surah'" x-cloak class="bg-amber-50/30 dark:bg-amber-950/20 p-3 rounded-lg border border-amber-100 dark:border-amber-900/30 space-y-3">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="number" id="ayah_start" name="ayah_start" :required="examType === 'surah'" min="1" value="{{ old('ayah_start') }}" class="block w-full rounded-lg border-zinc-300 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white text-sm focus:border-indigo-500 focus:ring-indigo-500 transition" />
                                </div>
                                <div>
                                    <label for="ayah_end" class="block text-xs font-bold text-zinc-600 dark:text-zinc-400 uppercase mb-1.5">Ayat Selesai *</label>
                                    <input type="number" id="ayah_end" name="ayah_end" :required="examType === 'surah'" min="1" value="{{ old('ayah_end') }}" class="block w-full rounded-lg border-zinc-300 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white text-sm focus:border-indigo-500 focus:ring-indigo-500 transition" />
                                </div>
                            </div>
                        </div>
    ...
    <script>
        function tahfizhExamApp() {
            return {
                ...
                validateForm(e) {
                    // Custom client-side validations if needed
                }
            };
        }
    </script>
