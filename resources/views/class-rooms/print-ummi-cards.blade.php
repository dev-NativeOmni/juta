...
<head>
    ...
    <title>Cetak Masal Kartu UMMI Kelas - {{ $classRoom->name }}</title>
...
<body class="p-4 sm:p-6 bg-zinc-100 min-h-screen text-zinc-900">
    ...
    <div class="no-print max-w-7xl mx-auto mb-6 flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-zinc-200">
        ...
        <div class="flex items-center gap-3">
            <button onclick="window.print()" class="inline-flex items-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-semibold shadow-sm transition">
                <x-heroicon-o-printer class="w-4 h-4" /> Cetak Semua Kartu
            </button>
    ...
    <div class="max-w-[1600px] mx-auto space-y-8 print:space-y-0">
        @foreach ($studentsData as $data)
            @php
                ...
                $latestUmmiRecord = $data['latestUmmiRecord'];
            @endphp
            <div class="print-page flex flex-col lg:flex-row justify-between gap-6 border-b pb-8 print:border-b-0 print:pb-0">
                @for ($c = 0; $c < 2; $c++)
                        ...
                        <div class="grid grid-cols-2 gap-x-6 gap-y-1.5 text-[10px] mb-4 border-b border-dashed pb-3">
                            <div class="space-y-1.5">
                                <div class="flex items-center">
                                    ...
                                    <span class="border-b border-dotted border-gray-400 flex-1 font-semibold whitespace-nowrap overflow-hidden text-ellipsis">{{ $student->name }}</span>
                                </div>
                                <div class="flex items-center">
